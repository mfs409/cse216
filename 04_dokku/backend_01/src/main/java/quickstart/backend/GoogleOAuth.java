...
public class GoogleOAuth {
    ...
    /** URI google redirects to after login event, equal to 'serverName + ":" + port + "/auth/google/callback"' */
     ...
     * creates the URI where google returns after an auth, the redirectUri, as well as
     * the URI to redirect a user to to log them in, the newAuthUrl.
     * 
     * @param serverName the host on which this is running, e.g. 'http://localhost'
     * @param port the port on which the webserver is running, e.g. 3000
     * @param clientId the web client id from https://console.cloud.google.com/
     * @param clientSecret the secret associated with clientId
     * @param authCallbackEndpoint the callback endpoint used by google after signin event, e.g. '/auth/google/callback'
     */
    public GoogleOAuth(String serverName, int port, String clientId, String clientSecret, String authCallbackEndpoint ){
        ...
        this.flow = new GoogleAuthorizationCodeFlow
                .Builder(httpTransport, GsonFactory.getDefaultInstance(), clientId, clientSecret, gApis)
